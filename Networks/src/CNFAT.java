...
public class CNFAT {
    public static void main(String[] args) throws IOException{
        ...
        byte [] year=n.getBytes(StandardCharsets.UTF_8);
        DatagramPacket dsend=new DatagramPacket(year,year.length,address,8080);
        ds.send(dsend);
        byte [] responce=new byte[1000];
        DatagramPacket drecieve=new DatagramPacket(responce,responce.length);
        ds.receive(drecieve);
        System.out.println("Leap Year: "+convert(responce));
    }
